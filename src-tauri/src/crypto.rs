...
pub fn encrypt_key_data(vault: &Mutex<Vault>, key_data: &str) -> Result<String, AppError> {
    let vault = vault
        .lock()
        .map_err(|e| AppError::Database(e.to_string()))?;
    let encrypted = vault
        .encrypt(key_data.as_bytes())
        .map_err(|e| AppError::Database(e))?;
    Ok(STANDARD.encode(&encrypted))
}

pub fn decrypt_key_data(vault: &Mutex<Vault>, encrypted_b64: &str) -> Result<String, AppError> {
    let encrypted = STANDARD
        .decode(encrypted_b64)
        .map_err(|e| AppError::Database(e.to_string()))?;
    let vault = vault
